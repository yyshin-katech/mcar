...
struct TRACK_CAN_WRITER_FD::find_struct TRACK_CAN_WRITER_FD::isIn_func(vector<track_info> v, track_info track_data){
  find_struct ret;
...
TRACK_CAN_WRITER_FD::TRACK_CAN_WRITER_FD(){
...
void TRACK_CAN_WRITER_FD::CALLBACK_INSPVA(const novatel_gps_msgs::InspvaConstPtr& data){
...
void TRACK_CAN_WRITER_FD::CALLBACK_TRACK(const perception_ros_msg::object_array_msg& data) {
    track_calling = true;
...
short TRACK_CAN_WRITER_FD::FIND_MSG_IDX(char* target_msg, vector<tuple<char*, vector<char*>>>* msg_list){
  short idx = -1;
...
void TRACK_CAN_WRITER_FD::GET_DATETIME(){
...
canStatus TRACK_CAN_WRITER_FD::OPEN_CAN_CHANNEL_AND_READ_DB(int channel_num, char *filename, bool init_access_flag){
...
float TRACK_CAN_WRITER_FD::MAHAL_DIST(const perception_ros_msg::object_array_msg& track_data){
...
void TRACK_CAN_WRITER_FD::CAN_MSG_PREPARE(int track_order, int msg_num) {
...
void TRACK_CAN_WRITER_FD::WRITE_CAN_MSG(vector<double> temp_data, char* track_msg){
...
int main(int argc, char **argv){
  ...
  ros::init(argc, argv, "Track_CAN_Writer_FD");
  ros::NodeHandle node("~");
  ...
  strcpy(filename, (relative_path + "/dbc/CANdb_IONIQev_PCAN2_FD.dbc").c_str());
  ...
  ros::Subscriber sub5 = node.subscribe("/track_Multi_RS",               1, &TRACK_CAN_WRITER_FD::CALLBACK_TRACK,       &TCW);
  ros::Subscriber sub6 = node.subscribe("/sensors/gps/inspva",           1, &TRACK_CAN_WRITER_FD::CALLBACK_INSPVA,       &TCW);
