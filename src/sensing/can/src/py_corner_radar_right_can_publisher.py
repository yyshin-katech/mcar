...
class CanListner(can.Listener):
    def __init__(self):
        ...
        # publisher
        self.sensor_pub = rospy.Publisher('/sensors/corner_radar_right', object_array_msg, queue_size=1)
    ...
    def sensor_publish(self):
        ...
        for i in range(len(self.sensor_msg)):
            ...
            temp_decoded = self.SODR[temp_idx].decode(data=temp_data)
        ...
        for i in range(len(self.sensor_polar)):
            ...
            right_angle = 90
            temp_msg.x = -1*temp_range * math.cos(-1*radians(temp_angle + right_angle))
            temp_msg.y = temp_range * math.sin(-1*radians(temp_angle + right_angle))

            temp_msg.vx = -1*temp_range_rate * math.cos(-1*radians(temp_angle + right_angle))
            temp_msg.vy = temp_range_rate * math.sin(-1*radians(temp_angle + right_angle))
    ...
    def on_message_received(self, msg):
        if msg.arbitration_id in self.SODR_id:
            i = self.SODR_id.index(msg.arbitration_id)
    ...
    def set_db(self):
        ...
        self.SODR_id = []
        for i in range(64):
            self.SODR.append(self.db.get_message_by_name('SODR_Track%02d'%(i+1)))
            self.SODR_id.append(self.SODR[i].frame_id)

if __name__ == '__main__':
    try:
        listener = CanListner()
        print " [*] DEFINE CAN LISTENER AND NOTIFIER ..."
        bus = can.interface.Bus(channel='can3', bustype='socketcan')
        listener.set_db()
