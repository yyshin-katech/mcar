...
class CrosswalkDetector {
 ...
 private:
  ...
  // --- 크로스워크 데이터 ---
  std::vector<Crosswalk> crosswalks_;   // id 1..9 순서
...
// crosswalk_data (1~9, EPSG:5179 폴리곤) — katech_ped_detector.py 값 그대로.
...
// CW_LINKS (크로스워크 N -> 접근 LINK_ID 집합) — katech_ped_detector.py 값 그대로. 링크셋 disjoint.
...
int CrosswalkDetector::membershipCrosswalk(double abs_e, double abs_n) const {
  // 전체 9개 순회, host_link_id ∈ CW_LINKS[N] 인 것만 ray-cast. 링크셋 disjoint → 0/1개.
