...
Reads HD map shapefiles (UTM Zone 52N) and publishes them as
...
Parameters:
  ~mapdir               directory containing the 11 HD map *.shp layers
                        (default: $(find gps_system_localizer)/src/shp_map/
                        senario_shp_20260623)
"""
import json
import math
import os
...
# into that organized layout (row*width + col). siheung_release uses
# /fusion_lidar_points instead — same point count but a different ordering,
# which scatters points to random positions on this platform. Overridable via
# the ~cloud_topic param.
# We cache the latest scan as a (N,3) float32 numpy slice and fan it out
# across the 10 Hz percept stream.
DEFAULT_CLOUD_TOPIC = "/percept_origin_rviz"
...
# Source CRS for senario_shp_20260623 (.prj says WGS_1984_UTM_Zone_52N).
SRC_EPSG = "EPSG:32652"
DST_EPSG = "EPSG:5179"

# Layer name -> kind. senario_shp_20260623 (MOLIT HD맵 11 표준 레이어, EPSG:32652).
# shape kinds verified via pyshp on src/shp_map/senario_shp_20260623/.
LAYERS_ALL = {
    "A1_NODE":                    "point",     # POINTZ     —    2859 nodes
    "A2_LINK":                    "polyline",  # POLYLINEZ  —    3014 lanelinks
    "A3_DRIVEWAYSECTION":         "polygon",   # POLYGONZ   —       8 driveway sections
    "B1_SAFETYSIGN":              "point",     # POINTZ     —     811 signs
    "B2_SURFACELINEMARK":         "polyline",  # POLYLINEZ  —    4670 lane marks
    "B3_SURFACEMARK":             "polygon",   # POLYGONZ   —    1374 surface marks
    "C1_TRAFFICLIGHT":            "point",     # POINTZ     —     625 traffic lights
    "C3_VEHICLEPROTECTIONSAFETY": "polyline",  # POLYLINEZ  —    1317 guardrails
    "C4_SPEEDBUMP":               "polygon",   # POLYGONZ   —      32 speed bumps
    "C5_HEIGHTBARRIER":           "polyline",  # POLYLINEZ  —      16 height barriers
    "C6_POSTPOINT":               "point",     # POINTZ     —     769 post points
}
    ...
    def __init__(self):
        ...
        self._mapdir = rospy.get_param(
            ...
            os.path.join(
                ...
                "src", "shp_map", "senario_shp_20260623",
            ),
        )
        ...
        # Point-cloud source topic. Default is this platform's (IONIQ 5)
        # organized cloud; siheung_release uses /fusion_lidar_points.
        self._cloud_topic = rospy.get_param("~cloud_topic", DEFAULT_CLOUD_TOPIC)
        self._tracks_count = 0
        self._with_points_count = 0
        self._cloud_xyz = None  # (N,3) float32, latest ~cloud_topic scan
        self._cloud_n = 0
