...
//                               (default /percept_origin_rviz — the K-City /
//                               IONIQ 5 organized cloud that cloud_indices
//                               index into. siheung_release uses
//                               /fusion_lidar_points, whose ordering differs.)
...
public:
  explicit WebHmiThreejsTracksCpp(ros::NodeHandle& nh) {
    ...
    const std::string cloud_topic =
        pnh.param<std::string>("cloud_topic", "/percept_origin_rviz");
...
private:
  ...
  void onCloud(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    // until the next point-cloud message arrives.
    cloud_msg_ = msg;
    cloud_n_ = n;
