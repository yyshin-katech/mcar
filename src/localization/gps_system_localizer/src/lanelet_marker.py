...
# MAPFILE_PATH = '/home/yuyeong/mcar_v13/src/localization/gps_system_localizer/src'
...
class lanelet_marker(object):
    ...
    def create_markers(self):
        ...
        for wp in self.wps:
            ...
            if point_count > 1000:
                step = max(1, point_count // 200)  # 최대 500포인트로 제한
            ...
            for i in range(0, len(wp['e']), step):
                try:
                    ...
                    # 좌표 유효성 검사
                    if abs(point.x) < 5000 and abs(point.y) < 5000:  # 50km 범위 내
                        marker_single.points.append(point)
                        valid_points += 1
