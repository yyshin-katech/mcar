...
- **crosswalk 좌표/CAN/게이팅**: katech_ped_detector.py `crosswalk_data`=EPSG:5179 폴리곤(오프라인 pyproj 4326→5179 리터럴). 검출 `/katech_msg/crosswalk_detection` → `katech_ped_detector_can_writer` CAN `Pedestrian_Stat`(528/529) `on_crosswalk=1`(crosswalk_id 없음·객체 4 cap). **검출을 ego LINK_ID(`CW_LINKS`, detector cpp·py·fusion 공유)로 게이팅 → 접근 크로스워크만**. occupancy_msg `uint8[] occupied_ids` 추가, fusion active(own=occupied_ids, obu #1·#2만). **크로스워크 1~18**(2026-07-27, 10~18 은 md 링크표 없어 mat `is_stop_line`+선행 20m 규칙으로 도출; #15/#16 만 접근링크 661/662 공유). web_hmi CrosswalkZones=[E,N] 1~18 점멸, mat 뷰어 var CROSSWALK=[lat,lon].
