...
- `rviz_filter.cpp` ROI 시각화: `percept_callback` 끝에 LINE_STRIP 마커 5개 (`ns="roi"`, id 0~4, ego_frame, lifetime 1s) 추가. A=cyan, B 전방측면=orange, C 후방측면=yellow. 측면 외곽 y는 ±10m로 표현. ROI 상수(`80/40/5/5`)는 percept_topic_matcher.cpp 와 동일 값으로 인라인 — 두 파일 동시 갱신 필수.
